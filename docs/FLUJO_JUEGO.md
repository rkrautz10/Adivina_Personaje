...
- `GET /rounds/:roundId/image` entrega una silueta PNG generada desde el canal
    alfa mientras la ronda esta `ACTIVE`, la imagen original cuando esta
    `RESOLVED` y `409 / CONFLICT` cuando esta `EXPIRED`; una ronda abandonada no
    revela la entidad.
